    ...
    for result in tqdm(results, desc="Aggregating results", total=len(results)):
        ...
        if not agg_result:
            agg_result = {
                ...
                "plausible_sentence": result["plausible_sentence"],
                "implausible_sentence": result["implausible_sentence"],
                "knowledge": [{"id": kg["id"], "head": kg["head"], "relation": kg["relation"], "tail": kg["tail"], "votes": int(kg.get("selected", False))} for kg in result["knowledge"]],
