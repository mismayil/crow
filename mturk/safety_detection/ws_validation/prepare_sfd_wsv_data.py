...
def main():
    ...
    for sample in tqdm(data, desc="Preparing data", total=len(data)):
        if not sample["no_alt_target"]:
            safe_actions = [action for action in sample["safe_actions"] if action["id"] == sample["action_id"]]
            unsafe_actions = [action for action in sample["unsafe_actions"] if action["id"] == sample["action_id"]]
            mturk_sample = {
                "id": sample["data_id"],
                ...
                "action": safe_actions[0]["text"] if safe_actions else unsafe_actions[0]["text"],
                "action_value": "safe" if safe_actions else "unsafe",
    ...
    output_path_stem = f"{args.output_dir}/mturk_data_sfd_wsv{args.suffix}"
