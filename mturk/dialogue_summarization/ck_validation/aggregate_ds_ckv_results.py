    ...
    for result in tqdm(results, desc="Aggregating results", total=len(results)):
        ...
        if not agg_result:
            agg_result = {
                ...
                "knowledge": [{"id": kg["id"], "head": kg["head"], "relation": kg["relation"], "tail": kg["tail"], "head_from": kg.get("head_from"), "tail_from": kg.get("tail_from"), "votes": int(kg.get("selected", False))} for kg in result["knowledge"]],
