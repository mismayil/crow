    ...
    for result in tqdm(results, desc="Aggregating results", total=len(results)):
        ...
        if not agg_result:
            agg_result = {
                ...
                "alt_targets": [{"id": target["id"], "text": target["text"], "perturbation": target["perturbation"], "votes": int(target.get("selected", False))} for target in result["alt_targets"]],
            ...
            for t_index, target in enumerate(result["alt_targets"]):
                agg_result["alt_targets"][t_index]["votes"] += int(target.get("selected", False))
